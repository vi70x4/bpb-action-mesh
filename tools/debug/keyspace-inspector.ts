...
function makePrefix(network: string, kind: string): string {
  return `/bpb/v2/${network}/${kind}/`;
}

function extractPeerIdFromKey(key: string): string {
  const parts = key.split("/");
  return parts[parts.length - 1];
}

function classifyKey(key: string): "vless" | "hysteria2" | "tombstone" | null {
...
async function scanFromFile(filePath: string, network: string): Promise<ScanResult> {
...
function isExpired(record: DHTRecord): boolean {
  try {
...
function expiresIn(record: DHTRecord): string {
  try {
...
function formatEntry(
  ...
  if (kind === "tombstone" && value) {
    ...
    const reason = ts.successor
      ? `successor: ${ts.successor}`
      : "TTL expired";
    return [
      `🪦 ${key}`,
...
function computeSummary(entries: ScanEntry[], tombstonePeerIds: Set<string>): Summary {
  let vless = 0;
  let hysteria2 = 0;
  let tombstones = 0;
  let orphans = 0;
...
async function main(): Promise<number> {
  ...
  if (opts.file) {
    ...
    try {
      ...
      const prefixes: Array<{ kind: "vless" | "hysteria2" | "tombstone"; prefix: string }> = [
  ...
  // ── Output ─────────────────────────────────────────────────────
  if (opts.json) {
    const json = buildJsonOutput(opts.network, result.entries, result.tombstonePeerIds);
    ...
    console.log(formatSummary(computeSummary(result.entries, result.tombstonePeerIds)));
  }
