...
function parseArgs(argv: string[]): { specPath: string; srcPath: string } {
  const defaults = {
...
function readSourceFiles(srcDir: string): Map<string, string> {
...
function checkDHTKeyPrefix(files: Map<string, string>): CheckResult {
  const prefix = "/bpb/v2/";
  const found: string[] = [];
...
function checkTombstoneFormat(files: Map<string, string>): CheckResult {
...
function checkRecordFields(files: Map<string, string>): CheckResult {
  const required = ["peerId", "protocol", "host", "port", "ttl", "bornAt", "expiresAt"];
  ...
  return {
    ...
    label: "Record fields",
    detail: `${found.length}/${required.length} required fields referenced` +
      (missing.length > 0 ? ` (missing: ${missing.join(", ")})` : ""),
  };
}

function checkTTLBounds(files: Map<string, string>): CheckResult {
  const allCode = [...files.values()].join("\n");
...
function checkNoSerialMultiHop(files: Map<string, string>): CheckResult {
